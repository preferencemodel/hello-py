...
class ComplexFeatureExtractor:
    ...
    def extract_gradient_features(self, data):
        ...
        grad_x = np.diff(images, axis=2)  # Shape: (n_samples, 28, 27)
        grad_y = np.diff(images, axis=1)  # Shape: (n_samples, 27, 28)
        ...
        features = np.column_stack([
            np.mean(abs_grad_x, axis=(1, 2)),
            np.std(abs_grad_x, axis=(1, 2)),
            np.mean(abs_grad_y, axis=(1, 2)),
            np.std(abs_grad_y, axis=(1, 2))
        ])
        return features
    ...
    def custom_normalize(self, features):
        ...
        distances_sq = np.maximum(distances_sq, 0)  # Avoid negative values from numerical errors
        distances = np.sqrt(distances_sq)
        
        # Compute median distances for each sample
...
class FastCustomActivation(nn.Module):
    ...
    def forward(self, x):
        ...
        # Apply different transformations based on sign
...
class ComplexNN(nn.Module):
    """
    Neural network with optimized custom architecture.
    """
