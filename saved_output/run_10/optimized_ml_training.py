...
This file implements an optimized ML training pipeline with vectorization.
"""
...
class ComplexFeatureExtractor:
    ...
    def extract_statistical_features(self, data):
        ...
        median_abs_dev = np.median(np.abs(data - mean), axis=1)
        
        features = np.column_stack([mean.flatten(), std.flatten(), max_val, min_val, median_abs_dev])
        return features
    ...
    def extract_frequency_features(self, data):
        ...
        # Percentile 95 for each sample
        fft_magnitude_flat = fft_magnitude.reshape(fft_magnitude.shape[0], -1)
        features[:, 3] = np.percentile(fft_magnitude_flat, 95, axis=1)
        
        return features

    def custom_normalize(self, features):
        """
        Optimized normalization using vectorized distance calculations.
        Uses standard scaling instead of pairwise median distances.
        """
        # Use simpler scaling: (features - mean) / std
...
class SlowCustomActivation(nn.Module):
    ...
    def forward(self, x):
        ...
        # Positive values: val * 0.9 + tanh(val) * 0.1
        output[pos_mask] = x[pos_mask] * 0.9 + torch.tanh(x[pos_mask]) * 0.1
        
        # Negative values: tanh(val) * 0.5
...
class SlowDataset(Dataset):
    ...
    def __init__(self, data, labels):
        self.data = data
        self.labels = labels
        # Pre-compute noise for deterministic behavior
        self.noise = np.random.randn(*data.shape) * 0.01
